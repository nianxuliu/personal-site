...
public void listenOrderTimeout(String orderNo) {
  ...
  },
  {
    ...
    title: "边缘端实时追踪实录：OpenCV + YOLO + CSRT 的落地细。",
    ...
    sections: [
      {
        ...
        paragraphs: [
          "我的工程入口不是“先追踪”，而是先把运行环境区分清楚。\n`track.py` 里先尝试导入 `hobot_dnn`，能。BPU 就走 BPU，失败再回退。`ultralytics`。\n这样做的目的不是炫技，而是保证开发板。PC 调试流程用同一套状态机逻辑。\n同一个模块在两端都能跑，调参和故障复现才不会分叉。\n这个结构也直接决定了后面 `_run_yolo_inference` 必须做统一适配层。",
          "我在参数层也把关键约束收敛在类初始化里，包括检测阈值、重确认频率、丢失容忍和惯性航行时长。\n这些参数不是装饰，它们分别控制了误检成本、漂移成本和控制抖动成本。\n比如 `RECONFIRM_INTERVAL=10` 对应。10 帧身份复核，`TRACKING_LOSS_TOLERANCE=10` 对应短时丢帧容忍。\n如果这些阈值散落在函数内部，后期定位“为什么误触发/为什么误重置”会非常痛苦。\n所以我把“可调参”当成工程一等公民，而不是补丁。",
        ],
        codeLanguage: "python",
        code: `try:
...
class HybridTracker:
    def __init__(self, model_path='best.pt'):
      ...
      {
        heading: "2. 发现阶段：光流海。+ ROI 。YOLO 确认",
        paragraphs: [
          "在未追踪状态下，我没有直接全图 YOLO，而是先做稠密光流把运动区域筛出来。\n真正。YOLO 推理只在 `roi = frame[y:y+h, x:x+w]` 上跑，这一步显著压低了每帧成本。\n这也是我能在边缘端保持实时性的关键动作，不是模型本身有多轻。\n如果拿全。YOLO 硬跑，哪怕检测准确率高，也很容易把控制链路拖慢。\n对机器人而言，慢半拍通常比“偶发漏检”更致命。",
  ...
  },
  {
    ...
    title: "IROS 任务代码复盘：人形机器人状态机与赛场执行链。",
    ...
    status: "Published",
    intro:
      "这篇我按我新增的 IROS 代码重新写了，不走泛化叙述，直接对着 `main_mission.py` 拆。\n会期信息先对齐：证书里写。IROS 2025 在杭州举办，时间。2025-10-19 。2025-10-25。\n在这个背景下，我的人形机器人程序核心不是单动作，而是状态机驱动的任务串联。\n所以正文重点放在状态切换、命令接入、导航校准、投放定位和摔倒恢复。\n所有代码段都来自本机真实文件，不补伪代码。",
    referenceDocs: [
      { label: "任务主代。main_mission.py", href: "/references/iros/main_mission.py" },
      { label: "赛项竞赛文档 PDF", href: "/references/iros/competition-rules.pdf" },
      { label: "报名。", href: "/references/iros/application.txt" },
      { label: "参会证明", href: "/references/iros/attendance.txt" },
    ],
    sections: [
      {
        heading: "1. 任务入口先定状态机：把赛程动作拆成可执行阶。",
        paragraphs: [
          "我看我这版代码最关键的一点，是先把任务拆成状态，再写每个状态动作。\n`WAIT_FOR_COMMAND -> MISSION_2 -> M3_START -> M3_NAV_A ... -> M3_FINISH` 这条主线非常清晰。\n这比把动作脚本堆在一起稳得多，因为每一步都有明确进入和退出条件。\n现场调试时，一旦某段出问题，可以直接定位在具体状态，而不是全链路盲查。\n竞赛里这类“可诊断结构”通常比单次动作炫技更有价值。",
    ...
    while current_state != State.FINISHED:
      ...
      {
        heading: "2. 命令接入。000 端口收车型机器人任务",
        paragraphs: [
          "我的命令接入不是硬编码，而是。socket，这样车型机器人和人形机器人之间是可替换协议层。\n`_wait_for_vehicle_command` 里绑。`0.0.0.0:6000`，收到数据后按两个字符一组解析。\n解析结果会映射成 `(颜色, 平台)` 任务列表，直接喂给后续状态机。\n如果超时或解析失败，还有默认后备任务，避免主流程直接崩。\n这就是竞赛程序里常说的“通信不稳定时仍可继续执行”。",
          "这里我觉得我写得比较稳的一点是白名单映射。\n`COLOR_MAP = {'。:'红色','。:'绿色','。:'蓝色'}` 先做语义归一，再进任务层。\n这样后面抓取和投放逻辑都只处理统一颜色词，不会被上游文本差异污染。\n我还支持 `USE_MOCK_COMMANDS`，现场断链时能立刻切调试模式。\n对比赛这种高压场景，这是很有价值的兜底。",
        ],
        codeLanguage: "python",
    ...
    return tasks`,
      ...
      {
        heading: "3. 任务二障碍穿越：动作序列和航向校正交替执。",
        paragraphs: [
          ...
          "另外我把栏架位置做成配置。`HURDLE_POSITION`，支。LEFT/CENTER/RIGHT 三路线。\n这比硬编码路线更适配现场任务卡变化。\n同一套程序通过配置就能切不同路径，不需要临场改核心代码。\n在比赛里减少“临时改代码”本身就是稳定性收益。\n这一点和状态机框架是配套的。",
        ],
        codeLanguage: "python",
...
def correct_heading(self, with_grab=False):
    return self.turn(self.target_heading, with_grab=with_grab)`,
      ...
      {
        heading: "5. 放置区识别：三角度扫。+ 物理顺序融合",
        paragraphs: [
          "抓取后的投放环节里，我用。`_scan_for_baskets` 做多角度观测，这段非常有代表性。\n头部。`[60, 90, 120]` 三个角度扫描 AprilTag，把相机坐标转换到机体坐标。\n然后不是直接相信单次观测，而是做“物理顺。+ 间距约束 + 加权融合”。\n这一步能显著缓解视觉噪声，尤其是在比赛光照和遮挡不稳定时。\n属于典型的“把工程先验加入感知结果”。",
    ...
    for tag_id in raw_measurements:
      ...
      {
        heading: "6. 异常兜底：摔倒检测与状态恢复重。",
        paragraphs: [
          "我这版代码另一个很实用的点，是把摔倒检测做进了主执行路径，而不是赛后补救。\n`is_fallen` 用站立基准和实时欧拉角差值做判断，超过阈值直接触发异常。\n状态机里捕。`FallDownException` 后会等待自动爬起，再重新校准航向继续执行当前状态。\n这让系统具备“被打断后可恢复”的能力，而不是一次失败全盘结束。\n在真实比赛里，这类恢复逻辑往往比单次动作精度更能保分。",
          "此外我还有视觉服务安全关闭和强制刷新函数。\n它们通过子进。超时避免 API 卡死把主流程拖挂，这是很工程化的处理。\n机器人比赛里最怕“整机卡死”，所以这种隔离式调用非常必要。\n从结构看，我的核心思路是：主链路要可持续，外围服务可降级。\n这个原则我建议继续保留到后续版本。",
        ],
        codeLanguage: "python",
